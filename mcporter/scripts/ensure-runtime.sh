...
if [[ ! -e "$CACHE_ROOT" ]]; then
	if ! mkdir -m 700 "$CACHE_ROOT" 2>/dev/null; then
		[[ -d "$CACHE_ROOT" ]] || {
			echo "FAIL: cannot create cache root: $CACHE_ROOT" >&2
			exit 1
		}
	fi
fi
[[ -d "$CACHE_ROOT" ]] || {
	echo "FAIL: cache root is not a directory: $CACHE_ROOT" >&2
	exit 1
}
CACHE_OWNER="$(stat -c '%u' "$CACHE_ROOT")"
CACHE_MODE="$(stat -c '%a' "$CACHE_ROOT")"
[[ "$CACHE_OWNER" == "$UID_ACTUAL" ]] || {
	echo "FAIL: cache root owner mismatch: $CACHE_ROOT" >&2
	exit 1
}
[[ "$CACHE_MODE" == "700" ]] || {
	echo "FAIL: cache root mode must be 700: $CACHE_ROOT" >&2
	exit 1
}
...
[[ -x "$NODE_STAGE" && -f "$CLI_STAGE" ]] || {
	echo "FAIL: incomplete runtime archive" >&2
	exit 1
}
[[ "$("$NODE_STAGE" --version)" == "$NODE_VERSION" ]] || {
	echo "FAIL: Node version mismatch" >&2
	exit 1
}
[[ "$("$NODE_STAGE" "$CLI_STAGE" --version)" == "$MCPORTER_VERSION" ]] || {
	echo "FAIL: mcporter version mismatch" >&2
	exit 1
}
