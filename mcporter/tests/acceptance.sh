...
[[ -x "$ROOT/scripts/install-runtime.sh" ]] || {
	echo "FAIL: installer missing"
	exit 1
}
[[ -x "$ROOT/scripts/ensure-runtime.sh" ]] || {
	echo "FAIL: cache loader missing"
	exit 1
}
...
[[ "$NODE_ACTUAL" == "$NODE_VERSION" ]] || {
	echo "FAIL: expected $NODE_VERSION got $NODE_ACTUAL"
	exit 1
}
[[ "$MCPORTER_ACTUAL" == "$MCPORTER_VERSION" ]] || {
	echo "FAIL: expected $MCPORTER_VERSION got $MCPORTER_ACTUAL"
	exit 1
}
