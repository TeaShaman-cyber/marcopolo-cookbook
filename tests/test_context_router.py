...
def route(route_id="x", **overrides):
    value = {"id": route_id, "priority": 1, "match": {}, "inherits": [], "inhibits": [], "sections": []}
    value.update(overrides)
    return value

def manifest(routes):
    return {"schema_version": SCHEMA, "defaults": {"max_sections": 4, "fallback_route": "default.unknown"}, "routes": [route("default.unknown", priority=-1000), *routes]}
...
class ContextRouterManifestTest(unittest.TestCase):
    ...
    def test_duplicate_route_id_is_rejected(self):
        with self.assertRaisesRegex(ContextRoutingError, "ROUTE_ID_DUPLICATE"):
            validate_manifest(manifest([route("x"), route("x", priority=2)]), Path.cwd())
    ...
    def test_section_path_escape_is_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            value = manifest([route("x", sections=[{"path": escape_path, "heading": "## A"}])])
    ...
    def test_missing_section_file_is_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            value = manifest([route("x", sections=[{"path": "missing.md", "heading": "## A"}])])
    ...
    def test_missing_heading_is_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            value = manifest([route("x", sections=[{"path": "doc.md", "heading": "## A"}])])
    ...
    def test_ambiguous_heading_is_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            (root / "doc.md").write_text("## A\none\n## A\ntwo\n", encoding="utf-8")
            value = manifest([route("x", sections=[{"path": "doc.md", "heading": "## A"}])])
            with self.assertRaisesRegex(ContextRoutingError, "SECTION_HEADING_AMBIGUOUS"):
                validate_manifest(value, root)
