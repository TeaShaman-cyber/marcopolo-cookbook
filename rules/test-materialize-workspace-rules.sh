...
grep -Fq '## Repository currentness / canonical checkout' "$SOURCE"
grep -Fq 'git-worktree-currentness.sh' "$SOURCE"
grep -Fq 'A visible repository path and a clean working tree do not prove' "$SOURCE"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
TARGET="$TMP/RULES.md"
FAKE_BIN="$TMP/bin"
