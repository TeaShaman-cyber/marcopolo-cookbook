...
def add_layer_evidence(
    receipt, layer, evidence_class, source, provider, authority="observed", value=None
):
...
def classify_layers(receipt):
    ...
    for obs in receipt.get("observations", []):
        ...
        if not candidate:
            continue
        grouped.setdefault(layer, {}).setdefault(candidate, set()).add(
            obs.get("evidence_class")
        )
        if obs.get("authority") == "provider_owned":
            authoritative.setdefault(layer, set()).add(candidate)
    layers = {}
    for layer, candidates in grouped.items():
        candidate, classes = sorted(
            candidates.items(), key=lambda item: (-len(item[1]), item[0])
        )[0]
        count = len(classes)
        if layer == "application":
            ...
            classification, confidence = (
                "VERIFIED_PROVIDER",
                min(1.0, 0.8 + 0.05 * (count - 2)),
            )
        elif count >= 2:
            classification, confidence = (
                "LIKELY_PROVIDER",
                min(0.79, 0.5 + 0.1 * (count - 2)),
            )
        else:
    ...
    infra = {
        ...
        if name in {"serving", "dns", "registrar"} and entry.get("provider")
    }
    if len(infra) > 1:
        receipt["classification"], receipt["confidence"] = (
            "MULTI_PROVIDER_OR_PROXY",
            0.75,
        )
    elif "serving" in layers:
        receipt["classification"], receipt["confidence"] = (
            layers["serving"]["classification"],
            layers["serving"]["confidence"],
        )
    elif layers:
