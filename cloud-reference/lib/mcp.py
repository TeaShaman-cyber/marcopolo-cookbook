...
BLOCKED_WAF_TERMS = re.compile(
    r"\b(bypass|evade|evasion|circumvent|disable|defeat)\b", re.I
)
...
def _temporary_config(sources):
    ...
    for source in executable_sources(sources):
        cfg["mcpServers"][source["name"]] = {
            "baseUrl": source["endpoint"],
            "description": f"Reviewed cloud-reference source for {source['domain']}",
        }
    handle = tempfile.NamedTemporaryFile(
        "w", prefix="cloudref-mcporter-", suffix=".json", delete=False
    )
    try:
...
def check_source(source_name, timeout=15):
    ...
    receipt["raw_hashes"] = [
        h for h in (result.get("stdout_sha256"), result.get("stderr_sha256")) if h
    ]
    if result["returncode"] != 0:
        receipt["failure"] = {
            "type": result.get("failure", "REFERENCE_SOURCE_ERROR"),
            "message": result.get("stderr", "")[:300],
        }
    else:
        try:
            ...
            receipt["failure"] = {
                "type": "REFERENCE_PROTOCOL_DRIFT",
                "message": "mcporter list returned non-JSON",
            }
    return receipt
...
def query_reference(provider, observation, mode="docs", timeout=20):
    ...
    result = run_mcporter(
        ["call", f"{server}.{tool}", f"{key}:{query}"], timeout=timeout
    )
    ...
    receipt["observations"] = [
        {
            ...
            if provider in {"aws", "cloudflare", "microsoft"}
            else "observed",
        }
    ]
    return receipt
