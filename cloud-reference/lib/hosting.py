...
def collect_http(hostname, timeout=5, body_limit=4096):
    req = urllib.request.Request(
        f"https://{hostname}/",
        headers={"User-Agent": "marcopolo-cloud-reference/0.1", "Accept": "*/*"},
    )
    try:
...
def build_hosting_receipt(hostname, timeout=5, observation_time=None):
    ...
    try:
        ...
        try:
            ...
            if matches:
                add_observation(
                    receipt,
                    "network_provider_range",
                    "aws-ip-ranges",
                    {
                        "provider_candidate": "aws",
                        "matches": matches,
                        "createDate": aws_ranges.get("createDate"),
                    },
    ...
    try:
        ...
        if tls_candidate:
            add_observation(
                ...
                "tls_provider_marker",
                "target",
                {"provider_candidate": tls_candidate, "issuer": tls.get("issuer")},
                authority="observed",
            )
            add_layer_evidence(
                receipt, "serving", "tls_provider_marker", "target", tls_candidate
            )
    except Exception as e:
    ...
    try:
        ...
        if server_component:
            add_layer_evidence(
                receipt, "application", "http_server", "target", server_component
            )
        for header in http["headers"]:
            ...
            if cand:
                authority = (
                    "provider_owned"
                    if header in {"x-vercel-id", "cf-ray", "fly-request-id"}
                    else "observed"
                )
                add_observation(
                    receipt,
                    "provider_header",
                    "target",
                    {"provider_candidate": cand, "header": header},
                    authority=authority,
                )
    except Exception as e:
