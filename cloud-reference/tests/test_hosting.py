...
class HostingTests(unittest.TestCase):
    def test_rejects_url_and_shell_text(self):
        for value in [
            ...
            "mcp.marcopolo.dev;id",
            "mcp.marcopolo.dev/path",
            "mcp marcopolo.dev",
        ]:
    ...
    def test_provider_candidates_are_conservative(self):
        self.assertEqual(
            detect_provider_candidate("cname", "something.vercel-dns.com"), "vercel"
        )
...
class LayeredReceiptTests(unittest.TestCase):
    def test_build_receipt_emits_serving_and_application_layers(self):
        ...
        http = {
            "status": 401,
            "final_url": "https://mcp.marcopolo.dev/",
            "headers": {"server": "uvicorn"},
            "body_sha256": "sha256:" + "0" * 64,
            "body_excerpt": "",
        }
        with (
            patch("hosting.collect_dns", return_value=dns),
            patch(
                "hosting.fetch_aws_ranges", return_value=(ranges, "sha256:" + "1" * 64)
            ),
            patch("hosting.collect_tls", return_value=tls),
            patch("hosting.collect_http", return_value=http),
        ):
            receipt = build_hosting_receipt(
                "mcp.marcopolo.dev", observation_time="2026-09-03T00:00:00Z"
            )
        self.assertEqual(receipt["layers"]["serving"]["provider"], "aws")
        self.assertEqual(
            receipt["layers"]["serving"]["classification"], "VERIFIED_PROVIDER"
        )
