...
session_search_load_runtime() {
	...
	_ss_corpus_token_set=${SESSION_SEARCH_CORPUS_TOKEN+x}
	_ss_corpus_token=${SESSION_SEARCH_CORPUS_TOKEN-}
	_ss_local_projection_set=${SESSION_SEARCH_LOCAL_PROJECTION+x}
	_ss_local_projection=${SESSION_SEARCH_LOCAL_PROJECTION-}
	...
	if [ "$_ss_corpus_token_set" = x ]; then
		SESSION_SEARCH_CORPUS_TOKEN=$_ss_corpus_token
		export SESSION_SEARCH_CORPUS_TOKEN
	fi
	if [ "$_ss_local_projection_set" = x ]; then
		SESSION_SEARCH_LOCAL_PROJECTION=$_ss_local_projection
		export SESSION_SEARCH_LOCAL_PROJECTION
	fi
	...
	unset _ss_corpus_token_set _ss_corpus_token _ss_local_projection_set _ss_local_projection
